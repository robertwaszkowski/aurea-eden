export class BpmnToFluentConverter {
    ...
    // ─────────────────────────────────────────────────────────────
    // Cardinal port from src toward tgt using their BPMN coordinates.
    // Falls back to 'auto' when coordinates are unavailable.
    // ─────────────────────────────────────────────────────────────
    _sourcePort(srcEl, tgtEl) {
        if (srcEl.x === null || tgtEl.x === null) return 'auto';
    ...
    convert(xmlString) {
        ...
        for (const seq of this.sequences) {
            ...
            if (src && tgt && processedElementIds.has(src.id) && processedElementIds.has(tgt.id)) {
                ...
                const srcPort = srcOutgoing.length >= 2 ? this._sourcePort(src, tgt) : 'auto';
