export class BpmnToFluentConverter {
    ...
    convert(xmlString, options = {}) {
        ...
        const hidePrimary = cfg.hiddenBranches && cfg.hiddenBranches.has('PRIMARY');
        ...
        for (let i = 0; i < primaryPath.length; i++) {
            ...
            if (i > 0) {
                ...
                line += ``; // Starting anchor has no base position method
            }
            ...
            // Append initial anchor comment if not hidden
            if (i === 0 && !hidePrimary) {
        ...
        if (cfg.enablePhase2) {
            ...
            while (unprocessed.length > 0 && maxIterations-- > 0) {
                ...
                for (const elId of unprocessed) {
                    ...
                    if (validIncomingEdges.length > 0) {
                        ...
                        // Calculate column span for sorting
                        ...
                        branchObj.span = Math.abs(mergeCol - startCol) + 1; // absolute for iterative branches too
            ...
            // Sort all discovered branches by span length (shortest first)
            detectedBranches.sort((a, b) => a.span - b.span);

            // Append the sorted branches for the UI badges, preserving the Primary Path
            ...
            for (const branch of detectedBranches) {
                ...
                for (let i = 0; i < branch.nodes.length; i++) {
                    ...
                    const isHidden = cfg.hiddenBranches && cfg.hiddenBranches.has(branch.nodes[0]);
                    if (isHidden) {
