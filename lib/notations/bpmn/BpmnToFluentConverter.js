        ...
        while (unprocessed.length > 0 && maxIterations-- > 0) {
            ...
            for (const elId of unprocessed) {
                ...
                if (validIncomingEdges.length > 0) {
                    ...
                    // Determine placement relative to anchor using BPMN coordinates.
                    //
                    // Three cases:
                    //  • Same row (|Δy| < threshold): .positionRightOf  — horizontal sibling in a branch lane
                    //  • Element is above anchor:     .positionUpOf
                    //  • Element is below anchor:     .positionDownOf   (default)
                    //
                    // The Y_TOLERANCE (25px) covers minor BPMN authoring jitter where two
                    // elements in the same swim-lane have a slightly different y value.
