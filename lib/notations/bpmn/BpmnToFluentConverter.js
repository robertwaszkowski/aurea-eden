export class BpmnToFluentConverter {
    ...
    _getFluentMethod(element, elId) {
        ...
        // Simple types that need exact names (not auto-camelCase)
        switch (typeName) {
            ...
            default: {
                // Auto-capitalise: 'exclusiveGateway' -> 'addExclusiveGateway'
    ...
    convert(xmlString) {
        ...
        // For elements with exactly 2 outgoing sequence flows we assign explicit ports:
        //   main flow (target on primary path or same-row): East → West
        //   branch flow:                                    South → North
        // All other connections use 'auto' on both ends.
        generatedLines.push("\n// --- Step 3: Global Connectors ---");
        for (const seq of this.sequences) {
            ...
            if (src && tgt && processedElementIds.has(src.id) && processedElementIds.has(tgt.id)) {
                ...
                if (srcOutgoing.length === 2) {
                    // Determine which target is the main (horizontal) flow
                    const isMain = primaryPathSet.has(tgt.id)
                        || (tgt.y !== null && src.y !== null && Math.abs(tgt.y - src.y) <= 25);
                    srcPort = isMain ? 'E' : 'S';
                    tgtPort = isMain ? 'W' : 'N';
                }

                let line = `${src.codeVarName}.connectTo('${tgt.id}', '${srcPort}', '${tgtPort}'`;
                if (label) {
                    line += `, '${label}'`;
                }
                line += `);`;
