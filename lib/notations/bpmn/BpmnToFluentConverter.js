export class BpmnToFluentConverter {
    ...
    convert(xmlString) {
        ...
        // • Source has 2+ outgoing flows (gateways, diverging elements):
        //     use 'explicit' source port so the routing engine picks the best
        //     exit direction per flow, while 'auto' handles the target entry.
        // • All other connections: fully automatic ('auto', 'auto').
        generatedLines.push("\n// --- Step 3: Global Connectors ---");
        for (const seq of this.sequences) {
            ...
            if (src && tgt && processedElementIds.has(src.id) && processedElementIds.has(tgt.id)) {
                ...
                const srcPort = srcOutgoing.length >= 2 ? 'explicit' : 'auto';
