export class BpmnToFluentConverter {
    ...
    convert(xmlString, options = {}) {
        ...
        for (const seq of this.sequences) {
            ...
            connList.push({ srcId: src.id, tgtId: tgt.id, srcPort, tgtPort: 'auto', label });
        }

        this._spreadPorts(connList);
    ...
    /**
     * Spreads connections that share the same exit or entry cardinal port on an element
     * across the port sub-family (NNW / N / NNE, etc.) to prevent collinear connector segments.
     * Modifies each connection object's srcPort and tgtPort in-place.
     * @param {Array<{srcId,tgtId,srcPort,tgtPort,label}>} connections
     */
    _spreadPorts(connections) {
        const FAMILIES = {
            N: ['NNW', 'N', 'NNE'],
            S: ['SSW', 'S', 'SSE'],
            E: ['NEE', 'E', 'SEE'],
        ...
        const SUB_PORTS = new Set(['NNW', 'NNE', 'SSW', 'SSE', 'NEE', 'SEE', 'NWW', 'SWW']);
        ...
        // ── PASS 1: spread source-side port collisions ────────────────────────
        const srcGroups = new Map();
        for (const conn of connections) {
            if (conn.srcPort === 'auto' || !FAMILIES[conn.srcPort]) continue;
            const key = `${conn.srcId}::${conn.srcPort}`;
            if (!srcGroups.has(key)) srcGroups.set(key, []);
            srcGroups.get(key).push(conn);
        }
        for (const [, group] of srcGroups) {
            if (group.length <= 1) continue;
            const ports = pick(FAMILIES[group[0].srcPort], group.length);
            group.forEach((conn, i) => { conn.srcPort = ports[i]; });
        }

        // ── PASS 1.5: resolve 'auto' tgtPort when srcPort is now a sub-port ──
        // The library's resolvePorts() only handles cardinal ports; a sub-port source
        // requires an explicit target to avoid passing 'auto' that can't be resolved.
        for (const conn of connections) {
            if (conn.tgtPort === 'auto' && SUB_PORTS.has(conn.srcPort)) {
                conn.tgtPort = OPPOSITE[PARENT[conn.srcPort]];
            }
        }

        // ── PASS 2: spread target-side port collisions ────────────────────────
        const tgtGroups = new Map();
        for (const conn of connections) {
            if (conn.tgtPort === 'auto' || !FAMILIES[conn.tgtPort]) continue;
            const key = `${conn.tgtId}::${conn.tgtPort}`;
            if (!tgtGroups.has(key)) tgtGroups.set(key, []);
            tgtGroups.get(key).push(conn);
        }
        for (const [, group] of tgtGroups) {
            if (group.length <= 1) continue;
            const ports = pick(FAMILIES[group[0].tgtPort], group.length);
            group.forEach((conn, i) => { conn.tgtPort = ports[i]; });
        }
    }
