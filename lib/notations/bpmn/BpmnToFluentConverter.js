export class BpmnToFluentConverter {
    ...
    _generateTopology(options) {
        ...
        // Helper to spread out ports when multiple connectors attach to the same side of a node
        const _spreadPorts = (conns) => {
            ...
            // Distribute N => N, NNW, NNE, etc.
            const spread = (basePort, count, index) => {
                ...
                const available = dirs[basePort] || [basePort];
                return available[index % available.length];
            };

            for (const [key, items] of portMap.entries()) {
                if (items.length > 1) {
                    const basePort = key.split('::')[1];
                    items.forEach((item, idx) => {
                        const newPort = spread(basePort, items.length, idx);
