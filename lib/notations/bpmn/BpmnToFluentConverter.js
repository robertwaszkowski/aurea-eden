export class BpmnToFluentConverter {
    ...
    _sourcePort(srcEl, tgtEl, srcOutgoingIds, primaryPathSet) {
        // Rule 1: Up port for iterations (backward flow) ONLY
        ...
        if (mainTarget) {
            ...
            return 'auto'; // Branch targets geometric auto-selection (N or S depending on Up/Down placement)
        }
    ...
    // ─────────────────────────────────────────────────────────────
    // Cardinal port from src into tgt based on Gateway rules:
    // ─────────────────────────────────────────────────────────────
    _targetPort(srcEl, tgtEl, tgtIncomingIds, primaryPathSet) {
    ...
    convert(xmlString, options = {}) {
        ...
        if (detectedBranches.length > 0) {
            ...
            // Flag branches that should physically route upwards
        ...
        // Pre-compute incoming flow counts per element to serve as tgtIncomingIds
        const incomingFlows = new Map();
        for (const seq of this.sequences) {
            if (!incomingFlows.has(seq.targetRef)) incomingFlows.set(seq.targetRef, []);
            incomingFlows.get(seq.targetRef).push(seq.sourceRef);
        }

        // --- Stage 4: Connectors (with port-spreading to prevent collinear overlaps) ---
        generatedLines.push("\n// --- Connectors ---");
        const connList = [];
        for (const seq of this.sequences) {
            ...
            if (!src || !tgt || !processedElementIds.has(src.id) || !processedElementIds.has(tgt.id)) continue;
            const srcOutgoing = outgoingFlows.get(src.id) || [];
            let srcPort = srcOutgoing.length >= 2 ? this._sourcePort(src, tgt, srcOutgoing, primaryPathSet) : 'auto';
            const tgtIncoming = incomingFlows.get(tgt.id) || [];
            let tgtPort = tgtIncoming.length >= 2 ? this._targetPort(src, tgt, tgtIncoming, primaryPathSet) : 'auto';

            connList.push({ srcId: src.id, tgtId: tgt.id, srcPort, tgtPort, label });
        }

        for (const conn of connList) {
            let line = `diagram.connect('${conn.srcId}', '${conn.tgtId}', '${conn.srcPort}', '${conn.tgtPort}'`;
    ...
    _generateTopology(options) {
        ...
        // Topology connections: pre-compute explicit ports from known positions, then spread.
        lines.push('\n// --- Topology Connections ---');
        const topoConns = [];
        for (const seq of this.sequences) {
            if (!this.elements.get(seq.sourceRef) || !this.elements.get(seq.targetRef)) continue;
            const srcPos = positions.get(seq.sourceRef) || { x: 0, y: 0 };
            const tgtPos = positions.get(seq.targetRef) || { x: 0, y: 0 };
            const { srcPort, tgtPort } = this._computeGeometricPorts(srcPos, tgtPos);
            const label = this._cleanText(seq.name);
            topoConns.push({ srcId: seq.sourceRef, tgtId: seq.targetRef, srcPort, tgtPort, label });
        }

        // Helper to intelligently spread out ports when multiple connectors attach to the same side of a node
        const _spreadPorts = (conns) => {
            const portMap = new Map();
            for (const c of conns) {
                const spKey = c.srcId + '::' + c.srcPort;
                const tpKey = c.tgtId + '::' + c.tgtPort;
                if (!portMap.has(spKey)) portMap.set(spKey, []);
                portMap.get(spKey).push({ conn: c, side: 'src' });

                if (!portMap.has(tpKey)) portMap.set(tpKey, []);
                portMap.get(tpKey).push({ conn: c, side: 'tgt' });
            }

            // Distribute and return geometrically ordered array of ports
            // Output is strictly ordered Left->Right (for N/S) or Top->Bottom (for E/W)
            const getSpreadPorts = (basePort, count) => {
                if (count === 1) return [basePort];
                if (count === 2) {
                    if (basePort === 'N') return ['NNW', 'NNE'];
                    if (basePort === 'S') return ['SSW', 'SSE'];
                    if (basePort === 'E') return ['ENE', 'ESE'];
                    if (basePort === 'W') return ['WNW', 'WSW'];
                }
                if (count >= 3) {
                    if (basePort === 'N') return ['NNW', 'N', 'NNE'];
                    if (basePort === 'S') return ['SSW', 'S', 'SSE'];
                    if (basePort === 'E') return ['ENE', 'E', 'ESE'];
                    if (basePort === 'W') return ['WNW', 'W', 'WSW'];
                }
                return Array(count).fill(basePort); // Fallback
            };

            for (const [key, items] of portMap.entries()) {
                if (items.length > 1) {
                    const [nodeId, basePort] = key.split('::');
                    const targetPos = positions.get(nodeId) || { x: 0, y: 0 };

                    // Look up coordinates to perform geometric anti-crossing sort
                    items.forEach(item => {
                        const otherId = item.side === 'src' ? item.conn.tgtId : item.conn.srcId;
                        item.remotePos = positions.get(otherId) || { x: 0, y: 0 };
                    });

                    // Bucket by approach direction relative to the target face
                    let bucketNegative = []; // E.g., approaching from Left/Top
                    let bucketPositive = []; // E.g., approaching from Right/Bottom
                    let bucketNeutral = [];  // E.g., exact same coordinate

                    if (basePort === 'N' || basePort === 'S') {
                        items.forEach(item => {
                            if (item.remotePos.x < targetPos.x) bucketNegative.push(item);
                            else if (item.remotePos.x > targetPos.x) bucketPositive.push(item);
                            else bucketNeutral.push(item);
                        });
                        // Sort so closest origin is FIRST
                        bucketNegative.sort((a, b) => b.remotePos.x - a.remotePos.x); // descending
                        bucketPositive.sort((a, b) => a.remotePos.x - b.remotePos.x); // ascending
                        bucketNeutral.sort((a, b) => a.remotePos.y - b.remotePos.y);  // arbitrary tie-break
                    } else if (basePort === 'E' || basePort === 'W') {
                        items.forEach(item => {
                            if (item.remotePos.y < targetPos.y) bucketNegative.push(item);
                            else if (item.remotePos.y > targetPos.y) bucketPositive.push(item);
                            else bucketNeutral.push(item);
                        });
                        // Sort so closest origin is FIRST
                        bucketNegative.sort((a, b) => b.remotePos.y - a.remotePos.y); // descending
                        bucketPositive.sort((a, b) => a.remotePos.y - b.remotePos.y); // ascending
                        bucketNeutral.sort((a, b) => a.remotePos.x - b.remotePos.x);  // arbitrary tie-break
                    }

                    const ports = getSpreadPorts(basePort, items.length); console.log('Spreading for: ', nodeId, basePort); console.log('bucketNegative:', bucketNegative.map(i => i.conn.srcId + ' (x=' + i.remotePos.x + ')'));

                    // Assign ports: 
                    // Negative bucket takes first ports (e.g. SSW, then S)
                    // Positive bucket takes last ports (e.g. SSE)
                    // Neutral takes whatever is left in the middle
                    const finalOrder = [];
                    bucketNegative.forEach(item => finalOrder.push(item));
                    bucketNeutral.forEach(item => finalOrder.push(item));

                    // For the positive bucket (right side), since we want the CLOSEST to take the CLOSEST port,
                    // and our ports array is ordered left->right, the closest right-side origin (which is FIRST in bucketPositive)
                    // should take the FIRST available slot on the right side.
                    // Which means we process them normally, but pop from the END of the ports array!

                    const assignments = new Map();
                    // Process Negative + Neutral from left to right
                    let pIdx = 0;
                    finalOrder.forEach(item => {
                        assignments.set(item, ports[pIdx++]);
                    });

                    // Process Positive from inward out (right-to-left assigned to closest-to-furthest)
                    let pEndIdx = ports.length - 1;
                    bucketPositive.forEach(item => {
                        assignments.set(item, ports[pEndIdx--]);
                    });

                    console.log(`\nSPREAD LOG: Target ${nodeId} Port ${basePort}`);
                    console.log(`Target X: ${targetPos.x}, Y: ${targetPos.y}`);
                    items.forEach(item => {
                        const newPort = assignments.get(item);
                        const otherId = item.side === 'src' ? item.conn.tgtId : item.conn.srcId;
                        console.log(`  Source: ${otherId} (x=${item.remotePos.x}, y=${item.remotePos.y}) -> assigned to ${newPort}`);
                        if (item.side === 'src') item.conn.srcPort = newPort;
                        else item.conn.tgtPort = newPort;
                    });
                }
            }
        };

        _spreadPorts(topoConns);
        for (const conn of topoConns) {
            let line = `diagram.connect('${conn.srcId}', '${conn.tgtId}', '${conn.srcPort}', '${conn.tgtPort}'`;
            if (conn.label) line += `, '${conn.label}'`;
            line += `);`;
            lines.push(line);
    ...
    _parseXml(xmlString) {
        ...
        const ELEMENT_TYPES = ['startEvent', 'task', 'userTask', 'serviceTask', 'manualTask',
            ...
            'endEvent', 'intermediateCatchEvent', 'intermediateThrowEvent', 'boundaryEvent',
    ...
    _findPrimaryPath() {
        ...
        // Semantic Heuristic Scoring to break ties on mathematically equivalent path lengths
        const scorePath = (path) => {
            let score = path.length * 100; // Base structural trunk score
            ...
            if (endNode && endNode.hasName) {
                ...
                if (name.includes('approve') || name.includes('success') || name.includes('complete') || name.includes('accept') || name.includes('yes')) {
                    score += 50;
                }
                if (name.includes('reject') || name.includes('fail') || name.includes('cancel') || name.includes('error') || name.includes('no')) {
                    score -= 50;
                }
            }
            return score;
        ...
        // Return the highest scored path (the genuine main happy path)
    ...
    /**
     * Computes cardinal source/target ports from element positions.
     * Mirrors Element.resolvePorts() for use at code-generation time (topology stage).
     */
    _computeGeometricPorts(srcPos, tgtPos) {
