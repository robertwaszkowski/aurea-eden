...
class BpmnDiagram extends Diagram {
    ...
    buildDiagram(xmlDoc) {
        ...
        for (let i = 0; i < bpmnShapesToPosition.length; i++) {
            ...
            if (bounds) {
                ...
                if (bpmnElementName && bpmnElementName.trim() !== '') {
                    ...
                    // (Tasks and Subprocesses have their internal text added at creation, so we completely ignore external BPMNLabels for them)
                    if (!isTaskOrProcess) {
                        let offset;
        ...
        for (let i = 0; i < bpmnEdges.length; i++) {
            ...
            // Snap each segment to its dominant axis.
            // BPMN orthogonal routing never has genuine diagonal segments — any
            // off-axis deviation (e.g. y=209 vs y=210) is a tool artefact.
            // We walk the waypoints and force each point to share either the same X
            // or the same Y as its predecessor, based on which axis dominates.
            ...
            if (rawPoints.length > 0) {
                ...
                for (let j = 1; j < rawPoints.length; j++) {
                    ...
                    if (dx >= dy) {
                        // Predominantly horizontal — lock Y to previous point's Y
                        connectorPoints.push(new THREE.Vector2(cur.x, prev.y));
                    } else {
                        // Predominantly vertical — lock X to previous point's X
            ...
            if (sequenceFlowName) {
                ...
                if (hasExplicitBounds) {
                    ...
                    if (filteredPoints.length > 1) {
                        ...
                        labelPosition = new THREE.Vector3(filteredPoints[0].x, filteredPoints[0].y + 10, 0);
                        labelWidth = 100;
                        labelHeight = 0;
                    } else {
                        continue;
