    ...
    buildDiagram(xmlDoc) {
        ...
        for (let i = 0; i < tasks.length; i++) {
            ...
            this.addTask(taskId, width, height)
                .addWrappedText(name, undefined, undefined, 'center');
        }
                ...
                if (bpmnLabel.length > 0) {
                    ...
                    if (labelBounds) {
                        ...
                        if (bpmnElementName) {
                            ...
                            // element.position is already set at line 829
                            // element.position is a Vector3
                            const offset = new THREE.Vector3().subVectors(labelPosition, position);
                            // Ensure z-index is slightly above (text default is usually +3, let's keep z offset or overwrite it?)
                            // addWrappedText uses offset.z as default 3 if not specified? 
                            // The signature is addWrappedText(text, offset = new THREE.Vector3(0, 0, 3), size = 8)
                            offset.z = 3;

                            element.addWrappedText(bpmnElementName, offset, 7, 'center', labelWidth, labelHeight, 'top');
                        }
                    }
                }
            }
        ...
        for (let i = 0; i < bpmnEdges.length; i++) {
            ...
            if (sequenceFlowName) {
                ...
                anchor.addWrappedText(sequenceFlowName, new THREE.Vector3(0, 0, 3), 7, 'center', labelWidth, labelHeight, 'top');
            }
        }
        this.arrange();
    }
