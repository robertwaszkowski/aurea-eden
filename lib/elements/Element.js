...
class Element extends THREE.Mesh {
     ...
     * @returns {Element} This element for method chaining
     */
    addText(text, size = 9, align = 'left') {
        const textElement = new Element(this.elementId + '_text', new TextShape(text, size, align));
     ...
     * @returns {Element} This element for method chaining
     */
    addWrappedText(wrappedText, offset = new THREE.Vector3(0, 0, 3), size = 9, align = 'left', wrapWidth = null, wrapHeight = null, vAlign = 'center') {
        if (!wrappedText) {
        ...
        for (let i = 1; i < words.length; i++) {
            ...
            const testWidth = TextShape.getLineLength(testLine, size);
        ...
        const wrappedTextElement = new Element(this.elementId + '_text', new TextShape(finalWrappedText, size, align));
        ...
        if (vAlign === 'top' && wrapHeight) {
            ...
            // Default offset.y assumes Center of Label Box.
            // We want Top of Text (TextCenter + Height/2) to align with Top of Label Box (BoxCenter + BoxHeight/2).
            // Desired TextCenterY = (BoxCenterY + BoxHeight/2) - TextHeight/2
            // Relative to BoxCenterY (which is 0 in the local context of offset):
            // Shift = BoxHeight/2 - TextHeight/2
        ...
        const finalOffset = new THREE.Vector3(xOffset, offset.y, offset.z);
        this.texts.push({ element: wrappedTextElement, positionOffset: finalOffset });
        return this;
    }
    ...
    addBadge(content, position = 'top-right', size = IconDimensions.ICON_SIZE_SMALL, animated = false) {
        ...
        // If it is an SVG (raw content or URL) AND explicit animation is requested, we want it to spin
        ...
        if ((isSvg || isSvgUrl) && animated) {
            badgeElement.userData.spin = true;
        }



        const zOffset = 2; // Position badge higher than icon (1) and element (0)
