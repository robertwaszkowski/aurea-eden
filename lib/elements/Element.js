...
class Element extends THREE.Mesh {
    ...
    positionAt(position) {
        ...
        // Notify diagram of movement for reactive connector updates
        if (this.diagram) {
            this.diagram.notifyElementMoved(this);
        }
    ...
    /**
     * Hides the element and all its decorations (texts, icons, etc.) visually,
     * while preserving its underlying scene geometry so that alignment anchors
     * and bounding limits are not collapsed.
     * @returns {Element} This element for method chaining
     */
    hide() {
        this.visible = false;
        this.texts.forEach(t => t.element.visible = false);
        this.icons.forEach(i => i.element.visible = false);
        this.badges.forEach(b => b.element.visible = false);
        this.valueBars.forEach(b => b.element.visible = false);
        return this;
    }
    ...
    // ================================================================
    // Single-Axis Placement Methods (Composable)
    // ================================================================

    /**
     * Aligns this element's X center with the reference element's X center.
     * @param {string} elementId - ID of the reference element
     * @returns {Element} This element for method chaining
     */
    alignXWith(elementId) {
        const element = this.diagram.getElementById(elementId);
        this.setPosition(element.position.x, this.position.y, this.position.z);
        return this;
    }

    /**
     * Aligns this element's Y center with the reference element's Y center.
     * @param {string} elementId - ID of the reference element
     * @returns {Element} This element for method chaining
     */
    alignYWith(elementId) {
        const element = this.diagram.getElementById(elementId);
        this.setPosition(this.position.x, element.position.y, this.position.z);
        return this;
    }

    /**
     * Shifts this element's X to be right of the reference element, with standard gap.
     * @param {string} elementId - ID of the reference element
     * @returns {Element} This element for method chaining
     */
    shiftRightOf(elementId) {
        const element = this.diagram.getElementById(elementId);
        const elementWidth = element.getSize().x;
        const thisWidth = this.getSize().x;
        this.setPosition(
            element.position.x + elementWidth / 2 + DiagramDimensions.DISTANCE_BETWEEN_ELEMENTS + thisWidth / 2,
            this.position.y,
            this.position.z
        );
        return this;
    ...
    /**
     * Shifts this element's X to be left of the reference element, with standard gap.
     * @param {string} elementId - ID of the reference element
     * @returns {Element} This element for method chaining
     */
    shiftLeftOf(elementId) {
        const element = this.diagram.getElementById(elementId);
        const elementWidth = element.getSize().x;
        const thisWidth = this.getSize().x;
        this.setPosition(
    ...
    /**
     * Shifts this element's Y to be below the reference element, with standard gap.
     * @param {string} elementId - ID of the reference element
     * @returns {Element} This element for method chaining
     */
    shiftDownOf(elementId) {
        const element = this.diagram.getElementById(elementId);
        const elementHeight = element.getSize().y;
        const thisHeight = this.getSize().y;
        this.setPosition(
            this.position.x,
            element.position.y - elementHeight / 2 - DiagramDimensions.DISTANCE_BETWEEN_ELEMENTS - thisHeight / 2,
            this.position.z
        );
        return this;
    }
    ...
    /**
     * Shifts this element's Y to be above the reference element, with standard gap.
     * @param {string} elementId - ID of the reference element
     * @returns {Element} This element for method chaining
     */
    shiftUpOf(elementId) {
     ...
     */
    getNorthEastPoint() {
        if (this.type === 'DiamondShape') {
            return new THREE.Vector2(this.position.x + this.getSize().x / 4,
                this.position.y + this.getSize().y / 4);
        }
        if (this.type === 'CircleShape') {
            return new THREE.Vector2(this.position.x + (this.getSize().x * Math.SQRT2) / 4,
                this.position.y + (this.getSize().y * Math.SQRT2) / 4);
        }
        return new THREE.Vector2(this.position.x + this.getSize().x / 2,
            this.position.y + this.getSize().y / 2);
    }
    ...
    getNorthNorthEastPoint() {
        ...
        if (this.type === 'CircleShape') {
            return new THREE.Vector2(this.position.x + this.getSize().x / 4,
                this.position.y + (this.getSize().y * Math.sqrt(3)) / 4);
        }
        return new THREE.Vector2(this.position.x + this.getSize().x / 4,
            this.position.y + this.getSize().y / 2);
    }
    ...
    getNorthEastEastPoint() {
        ...
        if (this.type === 'CircleShape') {
            return new THREE.Vector2(this.position.x + (this.getSize().x * Math.sqrt(3)) / 4,
                this.position.y + this.getSize().y / 4);
        }
        return new THREE.Vector2(this.position.x + this.getSize().x / 2,
            this.position.y + this.getSize().y / 4);
    ...
    getPointPosition(position) {
        switch (position) {
            ...
            case 'ENE':
            case 'eastsoutheast':
                return this.getNorthEastEastPoint(); // getNorthEastEast is the internal name used for ENE
            case 'WNW':
            case 'westnorthwest':
                return this.getNorthWestWestPoint(); // getNorthWestWest is the internal name used for WNW
            case 'SSE':
            case 'southsoutheast':
            ...
            case 'ESE':
            case 'eastsoutheast':
                return this.getSouthEastEastPoint(); // getSouthEastEast is the internal name used for ESE
            case 'WSW':
            case 'westsouthwest':
                return this.getSouthWestWestPoint(); // getSouthWestWest is the internal name used for WSW
            case 'auto':
            case 'center':
     ...
     * @returns {Element} This element for method chaining
     */
    addWrappedText(wrappedText, offset = null, fontSize = null, align = null, wrapWidth = null, wrapHeight = null, vAlign = null, faceCamera = null, color = null) {
        if (!wrappedText) {
        ...
        const finalVAlign = vAlign || this.textStyle.vAlign || 'center';
        ...
        if (finalVAlign === 'top') {
            ...
            if (wrapHeight) {
                ...
                // Anchor the top of the text exactly to finalOffset.y, expanding downward
    ...
    /**
     * Adds a structured sprite-based label to the element.
     * Sprites automatically face the camera and can have backgrounds.
     * @param {string} text - The text to display
     * @param {THREE.Vector3} offset - Position offset
     * @param {Object} [parameters={}] - Additional parameters (color, bgColor, type)
     * @returns {THREE.Sprite} The created sprite
     */
    addSpriteLabel(text, offset, parameters = {}) {
        const {
            fontSize = 32,
            color = '#ffffff',
        ...
        // ULTRA-GENEROUS padding to ensure numbers of any length fit
        const horizontalPadding = scaledPadding * 3.5;
        const totalWidth = textWidth + horizontalPadding * 2 + (taskType ? iconSize + scaledPadding : 0);
        const tailHeight = 10 * resolutionScale;
        const mainHeight = scaledFontSize + scaledPadding * 2;
        const totalHeight = mainHeight + tailHeight;

        canvas.width = totalWidth;
        canvas.height = totalHeight;

        // Background (Bubble)
        this._drawBubblePath(context, totalWidth, mainHeight, tailHeight, scaledBorderRadius);
        context.fillStyle = bgColor;
        context.fill();

        if (borderColor) {
            context.strokeStyle = borderColor;
            context.lineWidth = borderWidth * resolutionScale;
            context.stroke();
        }

        // Content Centering
        const contentWidth = textWidth + (taskType ? iconSize + scaledPadding : 0);
        ...
        if (taskType) {
            ...
            const iconY = (mainHeight - iconSize) / 2;
            const iconColor = taskType === 'my' ? '#ffd700' : '#c0c0c0';
            context.fillStyle = iconColor;
            context.strokeStyle = 'rgba(0,0,0,0.3)';
            context.lineWidth = 1 * resolutionScale;

            this._drawStar(context, iconX + iconSize / 2, iconY + iconSize / 2, 5, iconSize / 2, iconSize / 4);
            context.fill();
            context.stroke();
        }
        ...
        const textX = contentStartX + (taskType ? iconSize + scaledPadding : 0) + textWidth / 2;
        context.fillText(text, textX, mainHeight / 2);
        ...
        // Scale sprite back down to world units
        const worldScale = 0.4 / resolutionScale;
        sprite.scale.set(totalWidth * worldScale, totalHeight * worldScale, 1);
        ...
        sprite.userData = {
            ...
            context,
            parameters: {
                scaledFontSize, scaledPadding, scaledBorderRadius,
                bgColor, taskType, color, resolutionScale,
                borderColor, borderWidth,
                mainHeight, tailHeight
            },
            text
        };

        if (this.diagram && this.diagram.scene) {
            this.diagram.scene.add(sprite);
        }

        this.valueBars.push({ element: sprite, positionOffset: offset.clone(), isSpriteLabel: true });

        return sprite;
    }

    /**
     * Updates an existing sprite label with new text or color.
     * @param {THREE.Sprite} sprite - The sprite to update
     * @param {string} text - New text
     * @param {THREE.Color|string} [color=null] - New text color
     */
    updateSpriteLabel(sprite, text, color = null) {
        ...
        // Measure text to check if canvas needs resizing
        context.font = `Bold ${scaledFontSize}px Arial`;
        const metrics = context.measureText(text);
        const textWidth = metrics.width;
        const iconSize = taskType ? scaledFontSize : 0;
        ...
        // Resize canvas if needed (to accommodate longer text)
        if (requiredWidth > canvas.width) {
            // 2. CRITICAL FIX: Create a completely NEW canvas element!
            ...
            // 4. Update sprite userData
            sprite.userData.canvas = canvas;
            sprite.userData.context = context;

            // Canvas context state is new, so we must restore the font
            context.font = `Bold ${scaledFontSize}px Arial`;

            // Update sprite scale to match new canvas width while maintaining world-unit ratio
            const worldScale = 0.4 / resolutionScale;
            sprite.scale.set(canvas.width * worldScale, (mainHeight + tailHeight) * worldScale, 1);

            // Recreate the texture to prevent WebGL dimension overflow
        ...
        // Redraw Text and Icon with centering
        const contentWidth = textWidth + (taskType ? iconSize + scaledPadding : 0);
        ...
        // Redraw Text
        context.fillStyle = color ? (color.getStyle ? color.getStyle() : color) : defaultColor;
        context.textAlign = 'center';
        context.textBaseline = 'middle';
        // To center the entire content group (icon + text), the text's center should be at the bubble's center.
        // The icon is drawn to the left of the text.
        // If taskType exists, the icon takes up space. The text needs to be shifted right by half the icon's width
        // to make the combined icon+text block centered.
        const textX = centerX + (taskType ? (iconSize + scaledPadding) / 2 : 0);
        context.fillText(text, textX, mainHeight / 2);
    ...
    _drawBubblePath(ctx, width, height, tailHeight, radius) {
        const tailWidth = tailHeight * 1.2;
        ...
        ctx.lineTo(centerX + tailWidth / 2, height);
        ctx.lineTo(centerX, height + tailHeight);
        ctx.lineTo(centerX - tailWidth / 2, height);
    ...
    /**
     * Adds a value bar definition for analysis mode.
     * Can be called multiple times to add multiple bars (element width is divided equally).
     *
     * Supports two equivalent call signatures:
     *   Positional: addValueBar(heightValue, colorValue?, colorsInverted?)
     *   Object:     addValueBar({ heightValue, colorValue?, colorsInverted? })
     *
     * @param {number|Object} heightValueOrDef
     *   - If a number: the bar height (0-100, normalized).
     *   - If an object: { heightValue, colorValue?, colorsInverted? }
     * @param {number} [colorValue] - (positional form) Color scale value (0-100). Defaults to heightValue.
     * @param {boolean} [colorsInverted=false] - (positional form) If true, higher colorValue = redder (worse).
     * @returns {Element} This element for method chaining
     */
    addValueBar(heightValueOrDef, colorValue, colorsInverted = false) {
        let barDef;

        if (heightValueOrDef !== null && typeof heightValueOrDef === 'object') {
            // Object form: { heightValue, colorValue?, colorsInverted? }
            const { heightValue, colorValue: cv, colorsInverted: ci = false } = heightValueOrDef;
            barDef = {
                heightValue,
                colorValue: cv !== undefined ? cv : heightValue,
                colorsInverted: ci
            };
        } else {
            // Positional form: (heightValue, colorValue?, colorsInverted?)
            const hv = heightValueOrDef;
            barDef = {
                heightValue: hv,
                colorValue: colorValue !== undefined ? colorValue : hv,
                colorsInverted
            };
        }

        // Initialize bars array if needed
        if (!this.parameters.bars) {
            this.parameters.bars = [];
        }
        this.parameters.bars.push(barDef);

        // Backward-compat alias: parameters.value = first bar's heightValue
        this.parameters.value = this.parameters.bars[0].heightValue;
    ...
    /**
     * Helper to automatically determine the best connection ports based on relative positions.
     * @param {Element} sourceElement 
     * @param {Element} targetElement 
     * @returns {Object} { sourcePort: 'N'|'S'|'E'|'W', targetPort: 'N'|'S'|'E'|'W' }
     */
    static determineAutoPorts(sourceElement, targetElement) {
        ...
        // THREE.js coordinate system (+Y is North/Up)
        if (angle >= 7 * PI_4 || angle < PI_4) {
            // Target is to the East (Right)
            return { sourcePort: 'E', targetPort: 'W' };
        } else if (angle >= PI_4 && angle < 3 * PI_4) {
            // Target is to the North (Up)
            return { sourcePort: 'N', targetPort: 'S' };
        } else if (angle >= 3 * PI_4 && angle < 5 * PI_4) {
            // Target is to the West (Left)
            return { sourcePort: 'W', targetPort: 'E' };
        } else {
            // Target is to the South (Down)
            return { sourcePort: 'S', targetPort: 'N' };
        }
    }

    /**
     * Helper to determine optimal auto ports when one or both ports are 'auto'.
     * If one port is explicitly defined, it tries to pick an orthogonal target port
     * to promote 1-elbow L-curves, unless they are perfectly aligned for a 0-elbow straight line.
     */
    static resolvePorts(sourceElement, targetElement, sourcePos, targetPos) {
        let finalSrc = sourcePos;
        let finalTgt = targetPos;

        const dx = targetElement.position.x - sourceElement.position.x;
        const dy = targetElement.position.y - sourceElement.position.y;

        const isAlignedX = Math.abs(dx) < 10;
        const isAlignedY = Math.abs(dy) < 10;

        const srcIsSubPort = finalSrc.length > 1 && finalSrc !== 'auto';
        const tgtIsSubPort = finalTgt.length > 1 && finalTgt !== 'auto';

        // Helper functions to get cardinal directions from sub-ports (e.g. 'NEE' -> 'E', 'NNW' -> 'N')
        const getBasePort = (port) => {
            if (!port || port === 'auto' || port.length === 1) return port;

            // East/West families: NEE, SEE, NWW, SWW
            const ewFamily = ['NEE', 'SEE', 'NWW', 'SWW'];
            if (ewFamily.includes(port)) return port.charAt(port.length - 1); // 'E' or 'W'

            // North/South families: NNW, NNE, SSW, SSE
            const nsFamily = ['NNW', 'NNE', 'SSW', 'SSE'];
            if (nsFamily.includes(port)) return port.charAt(0); // 'N' or 'S'

            return port.charAt(0); // Fallback
        };

        const baseSrc = getBasePort(finalSrc);
        const baseTgt = getBasePort(finalTgt);

        // If both are auto, intelligently pick orthogonal ports if diagonally offset
        if (finalSrc === 'auto' && finalTgt === 'auto') {
            if (isAlignedY) {
                finalSrc = dx > 0 ? 'E' : 'W';
                finalTgt = dx > 0 ? 'W' : 'E';
            } else if (isAlignedX) {
                ...
                // Diagonally offset: custom cyclic L-Curve logic
                if (dy > 0 && dx > 0) {
                    finalSrc = 'E'; finalTgt = 'S';
                } else if (dy < 0 && dx > 0) {
                    finalSrc = 'S'; finalTgt = 'W';
                } else if (dy > 0 && dx < 0) {
                    finalSrc = 'N'; finalTgt = 'E';
                } else {
                    finalSrc = 'W'; finalTgt = 'N';
                }
            }
            return { sourcePort: finalSrc, targetPort: finalTgt };
        }

        // Helper to check alignment tolerance (e.g. within 10 units means straight line)
        // If source is explicitly set, and target is auto
        if (finalSrc !== 'auto' && finalTgt === 'auto') {
            if (baseSrc === 'N' || baseSrc === 'S') {
                if (isAlignedX) finalTgt = baseSrc === 'N' ? 'S' : 'N'; // Straight line capable
                else finalTgt = dx > 0 ? 'W' : 'E'; // Promote L-Curve into side ports
            } else if (baseSrc === 'E' || baseSrc === 'W') {
                if (isAlignedY) finalTgt = baseSrc === 'E' ? 'W' : 'E'; // Straight line capable
                else finalTgt = dy > 0 ? 'S' : 'N'; // Promote L-Curve into top/bottom ports
            }
        }

        // If target is explicitly set, and source is auto
        if (finalTgt !== 'auto' && finalSrc === 'auto') {
            if (baseTgt === 'N' || baseTgt === 'S') {
                if (isAlignedX) finalSrc = baseTgt === 'N' ? 'S' : 'N';
                else finalSrc = dx > 0 ? 'E' : 'W';
            } else if (baseTgt === 'E' || baseTgt === 'W') {
                if (isAlignedY) finalSrc = baseTgt === 'E' ? 'W' : 'E';
                else finalSrc = dy > 0 ? 'N' : 'S';
            }
        }
     ...
     * @param {string} [sourcePosition='auto'] - Connection point on the source element ('N', 'S', 'E', 'W', or 'auto')
     * @param {string} [targetPosition='auto'] - Connection point on this element ('N', 'S', 'E', 'W', or 'auto')
     * @param {string} [label=null] - Optional text label to place on the connector
     ...
     */
    connectFrom(sourceElementId, sourcePosition = 'auto', targetPosition = 'auto', label = null, type = 'sequence') {
        if (!this.diagram) {
        ...
        const resolved = Element.resolvePorts(sourceElement, targetElement, sourcePosition, targetPosition);
        sourcePosition = resolved.sourcePort;
        targetPosition = resolved.targetPort;
        ...
        if (type === 'association') {
            ...
            // Default orthogonal
            const points = Connector.determinePoints(sourceElement, targetElement, sourcePosition, targetPosition);
            waypoints = points.slice(1, -1); // Extract inner waypoints, excluding source and target
    ...
    /**
     * Creates a connector from this element to another element.
     * @param {string} targetElementId - ID of the target element
     * @param {string} [sourcePosition='auto'] - Connection point on this element
     * @param {string} [targetPosition='auto'] - Connection point on the target element
     * @param {string} [label=null] - Optional text label to place on the connector
     * @param {string} [type='sequence'] - Type of connector ('sequence', 'association')
     * @returns {Element} This element for method chaining
     * @throws {Error} If the diagram is not set or target element is not found
     */
    connectTo(targetElementId, sourcePosition = 'auto', targetPosition = 'auto', label = null, type = 'sequence') {
        if (!this.diagram) {
            throw new Error("Diagram is not set for this element.");
        }

        const sourceElement = this;
        const targetElement = this.diagram.getElementById(targetElementId);
