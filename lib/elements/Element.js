...
class Element extends THREE.Mesh {
    ...
    static resolvePorts(sourceElement, targetElement, sourcePos, targetPos) {
        ...
        if (finalSrc !== 'auto' && finalTgt === 'auto') {
            if (finalSrc === 'N' || finalSrc === 'S') {
                if (isAlignedX) finalTgt = finalSrc === 'N' ? 'S' : 'N'; // Straight line capable
                else finalTgt = dx > 0 ? 'W' : 'E'; // Promote L-Curve into side ports
            } else if (finalSrc === 'E' || finalSrc === 'W') {
                if (isAlignedY) finalTgt = finalSrc === 'E' ? 'W' : 'E'; // Straight line capable
        ...
        if (finalTgt !== 'auto' && finalSrc === 'auto') {
            if (finalTgt === 'N' || finalTgt === 'S') {
                if (isAlignedX) finalSrc = finalTgt === 'N' ? 'S' : 'N';
                else finalSrc = dx > 0 ? 'E' : 'W';
            } else if (finalTgt === 'E' || finalTgt === 'W') {
                if (isAlignedY) finalSrc = finalTgt === 'E' ? 'W' : 'E';
    ...
    connectFrom(sourceElementId, sourcePosition = 'auto', targetPosition = 'auto', label = null, type = 'sequence') {
        ...
        const connector = new Connector(`connector-${sourceElement.elementId}-${targetElement.elementId}`, connectorShape, sourceElement, targetElement, sourcePoint, targetPoint, waypoints);
