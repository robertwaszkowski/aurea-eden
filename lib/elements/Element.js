...
class Element extends THREE.Mesh {
    ...
    addWrappedText(text) {
        ...
        const elementHeightZ = this.getSize().z || 1; // fallback to 1 if z is 0
        const textElevation = (elementHeightZ / 2) + 0.5;   // add a small offset above the element

        this.diagram.addElement(wrappedTextElement)
            .positionAt({ x: this.position.x, y: this.position.y, z: textElevation });

        // Add the wrapped text element to the texts array with a position offset
        this.texts.push({ element: wrappedTextElement, positionOffset: new THREE.Vector3(0, 0, textElevation) });
        return this;
    }
