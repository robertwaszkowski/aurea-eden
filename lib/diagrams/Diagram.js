        ...
        barsToAnimate.forEach(info => {
            ...
            const barTween = new Tween(barState)
                ...
                .onComplete(() => {
                    ...
                    // Stage 2: Label crashes down spectacularaly
                    const startZ = spriteLabel.position.z;
                    const labelState = { z: startZ };
                    const labelTween = new Tween(labelState)
                        .to({ z: 12 }, 1000) // Much slower crash for visibility
                        .delay(200) // The "spectacular hang"
                        .easing(Easing.Bounce.Out) // High-impact bounce
                        .onUpdate(() => {
                            spriteLabel.position.z = labelState.z;
                        })
                        .onComplete(() => {
                            animatedComponentsCount++; // Stage 2 complete

                            // Stage 3: Fade and disappear (400ms)
                            const fadeState = { opacity: 1 };
                            const fadeTween = new Tween(fadeState)
                                .to({ opacity: 0 }, 400)
                                .onUpdate(() => {
                                    if (spriteLabel.material) {
                                        spriteLabel.material.opacity = fadeState.opacity;
                                    }
                                })
                                .onComplete(() => {
                                    ...
                                    animatedComponentsCount++; // Stage 3 complete
                                    if (animatedComponentsCount >= totalComponents && onComplete) onComplete();
                                })
                                .start();

                            this.analysisTweens.push(fadeTween);
                        })
                        .start();
