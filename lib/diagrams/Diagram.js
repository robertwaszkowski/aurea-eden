...
class Diagram {
    ...
    fitScreen() {
        ...
        if (this.camera.isPerspectiveCamera) {
            ...
            this.camera.zoom = 1; // RESET ZOOM TO 1 FOR STABLE BOUNDS
    ...
    animateValueBarsOut(onComplete) {
            ...
            if (el.valueBars && el.valueBars.length > 0) {
                el.valueBars.forEach(barInfo => {
                    barsToAnimate.push({
                        ...
                        originalHeight: barInfo.element.scale.z * 100 // Approximation
                    });
                });
            }
        });
        ...
        let completedBars = 0;
        const totalBars = barsToAnimate.length;

        barsToAnimate.forEach(info => {
            const barElement = info.barElement;
            const textElement = (barElement.texts && barElement.texts.length > 0) ? barElement.texts[0].element : null;

            const animationState = { progress: 1 }; // Start from fully extended
            const startScaleZ = barElement.scale.z;
            const startTextZ = textElement ? textElement.position.z : 0;

            const tween = new Tween(animationState)
                .to({ progress: 0 }, 800) // 0.8 second sink
                .easing(Easing.Quartic.In)
                .onUpdate(() => {
                    const p = animationState.progress;
                    barElement.scale.z = startScaleZ * p;
                    if (textElement) {
                        // Sink text with bar
                        textElement.position.z = startTextZ * p;
                    }
                })
                .onComplete(() => {
                    completedBars++;
