...
class Diagram {
    ...
    _spreadPendingConnections() {
        ...
        for (const [key, group] of portGroups) {
            ...
            const [nodeId, cardinalPort] = key.split('::');
            const targetPos = this.getElementById(nodeId).position;
            const ports = pick(FAMILIES[cardinalPort], group.length);

            // Look up coordinates to perform geometric anti-crossing sort
            group.forEach(item => {
                const otherId = item.type === 'src' ? item.conn.targetId : item.conn.sourceId;
                const otherEl = this.getElementById(otherId);
                item.remotePos = otherEl ? otherEl.position : { x: 0, y: 0, z: 0 };
            });
            ...
            if (cardinalPort === 'N' || cardinalPort === 'S') {
                group.forEach(item => {
                ...
                bucketNeutral.sort((a, b) => a.remotePos.y - b.remotePos.y);
            } else if (cardinalPort === 'E' || cardinalPort === 'W') {
                group.forEach(item => {
                ...
                bucketNeutral.sort((a, b) => a.remotePos.x - b.remotePos.x);
            }
            ...
            group.forEach(item => {
                ...
                if (item.type === 'src') {
                    item.conn.sourcePort = newPort;
                } else {
                    item.conn.targetPort = newPort;
                }
            });
        }
    ...
    _initAnalyzeBars() {
        ...
        elementsWithBars.forEach(element => {
            ...
            bars.forEach((barDef, barIndex) => {
                ...
                // Safeguard: Skip generating the mesh entirely if height is 0 to prevent WebGL dimension overflow
                if (barHeight > 0) {
     ...
     * The physical anchor element is hidden but kept in the diagram so it participates in layout updates.
     * @private
     */
    _checkAndResolveAnchorPoint(anchorElement) {
        ...
        const mergedConnector = new Connector(
            ...
            incoming.type,
            {
                waypoints: [anchorElement], // Force passing through the Anchor Element instance!
                waypointPorts: [incoming.targetPosition, outgoing.sourcePosition] // Retain explicit routing ports
            }
        );

        mergedConnector.setDiagram(this);

        // 2. Remove the old connectors from the diagram. 
        // We DO NOT remove the anchor element. It remains to participate in resolveOverlaps positioning.
        this._removeConnector(incoming);
        this._removeConnector(outgoing);
        // this._removeElement(anchorElement); // NO! MUST STAY IN DIAGRAM.

        // Make sure it continues to be fully invisible
        anchorElement.hide();
        // Since we didn't remove the element, we should update connector mapping to point to the new single connector
        const anchorConns = this.elementConnectors.get(anchorElement.elementId);
        anchorConns.clear();
        anchorConns.add(mergedConnector);
    ...
    clear() {
        ...
        // Properly remove value bars and their sprite labels BEFORE clearing elements
        this.removeValueBars();
