...
class Diagram {
    ...
    calculateOptimalZoom() {
        // Get scene bounds from elements only
        const box = this.getDiagramBounds();
        console.log('calculateOptimalZoom: box', box);
        if (box.isEmpty() || !isFinite(box.min.x)) {
            console.log('calculateOptimalZoom: box is empty or invalid, returning default 500');
            return 500; // Default distance if empty
        }

        const size = box.getSize(new THREE.Vector3());
        console.log('calculateOptimalZoom: size', size);

        // Calculate aspect ratio and field of view
        // Use container dimensions, but ensure clientHeight is not zero
        ...
        let result = Math.max(distanceForWidth, distanceForHeight);

        if (!isFinite(result)) result = 500;

        console.log('calculateOptimalZoom: result', result);
        return result;
    }
    ...
    fitScreen() {
        ...
        if (!isFinite(cameraZ)) {
            console.warn('fitScreen: Calculated cameraZ is not finite:', cameraZ);
            return;
        }

        // Set camera position and orientation
        console.log(`fitScreen: Setting camera position to (0, 0, ${cameraZ})`);
    ...
    setTheme(themeName) {
        ...
        this.elements.forEach(el => {
            if (!el.themable) return;

            const strokeColor = (themeConfig.SEMANTIC && themeConfig.SEMANTIC[el.semanticType])
            ...
            if (el.texts) {
                el.texts.forEach(t => {
                    // Texts on themable elements follow theming usually
                    if (t.element && t.element.material && t.element.material.color) {
                        t.element.material.color.set(themeConfig.ELEMENT_TEXT);
                        if (t.element.material.emissive) {
                            t.element.material.emissive.set(themeConfig.ELEMENT_TEXT);
                        }
                    }
                });
            }
            ...
            case 'ANALYZE':
                ...
                this.rotate(-65, () => {
                    ...
                    this.elements.forEach(element => {
                        if (element.texts && element.texts.length > 0) {
                            element.texts.forEach(textInfo => {
                                ...
                                // Re-apply rotation to face camera if enabled
                                if (textInfo.faceCamera) {
                    ...
                    elementsWithValue.forEach(element => {
                        ...
                        barElement.addWrappedText("0" + suffix, textOffset, 12, 'center', null, null, 'center', true);
                        ...
                        const tween = new Tween(animationState)
                            ...
                            .onUpdate(() => {
                                ...
                                textElement.updateTextContent(currentValue.toString() + suffix);
                            })
                            .start();
